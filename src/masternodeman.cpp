...
bool CMasternodeDB::Write(const CMasternodeMan& mnodemanToSave)
{
    ...
    printf("Written info to mncache.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodeDB::ReadResult CMasternodeDB::Read(CMasternodeMan& mnodemanToLoad, bool fDryRun)
{
    ...
    printf("Loaded info from mncache.dat  %dms\n", GetTimeMillis() - nStart);
    printf("  %s\n", mnodemanToLoad.ToString());
    if (!fDryRun) {
        printf("Masternode manager - cleaning....\n");
        mnodemanToLoad.CheckAndRemove(true);
        printf("Masternode manager - result:\n");
        printf("  %s\n", mnodemanToLoad.ToString());
    }
...
void DumpMasternodes()
{
    ...
    printf("Verifying mncache.dat format...\n");
    ...
    printf("Writting info to mncache.dat...\n");
    mndb.Write(mnodeman);

    printf("Masternode dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
void CMasternodeMan::AskForMN(CNode* pnode, CTxIn& vin)
{
    ...
    printf("CMasternodeMan::AskForMN - Asking node for missing entry, vin: %s\n", vin.ToString());
        ...
        if (!(pnode->addr.IsRFC1918() || pnode->addr.IsLocal())) {
            ...
            if (it != mWeAskedForMasternodeList.end()) {
                if (GetTime() < (*it).second) {
                    printf("dseg - we already asked %s for the list; skipping...\n", pnode->addr.ToString());
                    return;
                }
            }
        }
...
CMasternode* CMasternodeMan::FindRandomNotInVec(std::vector<CTxIn>& vecToExclude, int protocolVersion)
{
    ...
    printf("CMasternodeMan::FindRandomNotInVec - nCountEnabled - vecToExclude.size() %d\n", nCountEnabled - vecToExclude.size());
    if (nCountEnabled - vecToExclude.size() < 1) return NULL;

    int rand = GetRandInt(nCountEnabled - vecToExclude.size());
    printf("CMasternodeMan::FindRandomNotInVec - rand %d\n", rand);
    bool found;
...
int CMasternodeMan::GetMasternodeRank(const CTxIn& vin, int64_t nBlockHeight, int minProtocol, bool fOnlyActive)
{
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        if (mn.protocolVersion < minProtocol) {
            printf("Skipping Masternode with obsolete version %d\n", mn.protocolVersion);
            continue;                                                       // Skip obsolete versions
        }
        ...
        if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
            nMasternode_Age = mn.lastPing.sigTime - mn.sigTime;
            if ((nMasternode_Age) < nMasternode_Min_Age) {
                printf("Skipping just activated Masternode. Age: %ld\n", nMasternode_Age);
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (!obfuScationSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubKeyCollateralAddress)) {
            printf("mnb - Got mismatched pubkey and vin\n");
        ...
        if (mnb.CheckInputsAndAdd(nDoS)) {
            ...
            printf("mnb - Rejected Masternode entry %s\n", mnb.addr.ToString());
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (!vin.scriptSig.empty()) {
            printf("dsee - Ignore Not Empty ScriptSig %s\n", vin.ToString());
            Misbehaving(pfrom->GetId(), 100);
            return;
        }
