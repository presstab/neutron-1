...
bool CMasternodePaymentDB::Write(const CMasternodePayments& objToSave)
{
    ...
    printf("Written info to mnpayments.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodePaymentDB::ReadResult CMasternodePaymentDB::Read(CMasternodePayments& objToLoad, bool fDryRun)
{
    ...
    printf("Loaded info from mnpayments.dat  %dms\n", GetTimeMillis() - nStart);
    printf("  %s\n", objToLoad.ToString());
    if (!fDryRun) {
        printf("Masternode payments manager - cleaning....\n");
        objToLoad.CleanPaymentList();
        printf("Masternode payments manager - result:\n");
        printf("  %s\n", objToLoad.ToString());
    }
...
void DumpMasternodePayments()
{
    ...
    printf("Verifying mnpayments.dat format...\n");
    ...
    if (readResult == CMasternodePaymentDB::FileError)
        printf("Missing budgets file - mnpayments.dat, will try to recreate\n");
    ...
    printf("Writting info to mnpayments.dat...\n");
...
bool IsBlockValueValid(const CBlock& block, int64_t nExpectedValue, CAmount nMinted)
{
    ...
    if (nHeight == 0) {
        printf("IsBlockValueValid() : WARNING: Couldn't find previous block\n");
    }

    //printf("XX69----------> IsBlockValueValid(): nMinted: %d, nExpectedValue: %d\n", FormatMoney(nMinted), FormatMoney(nExpectedValue));
    ...
    if (!masternodeSync.IsSynced()) { //there is no budget data to use to check anything
        ...
        if (nHeight % GetBudgetPaymentCycleBlocks() < 100) {
            ...
            if (nMinted > nExpectedValue) {
                printf("XX69-------> IsBlockValueValid(): Block value too much! Actual: %d, Expected: %d\n", FormatMoney(nMinted), FormatMoney(nExpectedValue));
                return false;
            }
        }
...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS)) {
        if (budget.IsBudgetPaymentBlock(nBlockHeight)) {
            ...
            printf("Invalid budget payment detected %s\n", txNew.ToString().c_str());
            if (IsSporkActive(SPORK_9_MASTERNODE_BUDGET_ENFORCEMENT))
                return false;

            printf("Budget enforcement is disabled, accepting block\n");
            return true;
        }
    }
    ...
    if (masternodePayments.IsTransactionValid(txNew, nBlockHeight))
        return true;
    printf("Invalid mn payment detected %s\n", txNew.ToString().c_str());

    if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT))
        return false;
    printf("Masternode payment enforcement is disabled, accepting block\n");
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    if (hasPayment) {
        ...
        printf("Masternode payment of %s to %s\n", FormatMoney(masternodePayment).c_str(), address2.ToString().c_str());
    }
}
...
bool CMasternodePaymentWinner::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, vchSig, keyMasternode)) {
        printf("CMasternodePing::Sign() - Error: %s\n", errorMessage.c_str());
        return false;
    }
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
        ...
        BOOST_FOREACH (CTxOut out, txNew.vout) {
            if (payee.scriptPubKey == out.scriptPubKey) {
                if(out.nValue >= requiredMasternodePayment)
                    ...
                    printf("Masternode payment is out of drift range. Paid=%s Min=%s\n", FormatMoney(out.nValue).c_str(), FormatMoney(requiredMasternodePayment).c_str());
            }
        }
    ...
    printf("CMasternodePayments::IsTransactionValid - Missing required payment of %s to %s\n", FormatMoney(requiredMasternodePayment).c_str(), strPayeesPossible.c_str());
    return false;
}
...
bool CMasternodePaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (!pmn) {
        ...
        printf("CMasternodePaymentWinner::IsValid - %s\n", strError);
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    if (budget.IsBudgetPaymentBlock(nBlockHeight)) {
        ...
        printf("CMasternodePayments::ProcessBlock() Start nHeight %d - vin %s. \n", nBlockHeight, activeMasternode.vin.ToString().c_str());
        ...
        if (pmn != NULL) {
            printf("CMasternodePayments::ProcessBlock() Found by FindOldestNotInVec \n");
            ...
            printf("CMasternodePayments::ProcessBlock() Winner payee %s nHeight %d. \n", address2.ToString().c_str(), newWinner.nBlockHeight);
        } else {
            printf("CMasternodePayments::ProcessBlock() Failed to find masternode to pay\n");
        }
    }
    ...
    if (!obfuScationSigner.SetKey(strMasterNodePrivKey, errorMessage, keyMasternode, pubKeyMasternode)) {
        printf("CMasternodePayments::ProcessBlock() - Error upon calling SetKey: %s\n", errorMessage.c_str());
        return false;
    }

    printf("CMasternodePayments::ProcessBlock() - Signing Winner\n");
    if (newWinner.Sign(keyMasternode, pubKeyMasternode)) {
        printf("CMasternodePayments::ProcessBlock() - AddWinningMasternode\n");
