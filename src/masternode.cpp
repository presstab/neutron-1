...
uint256 CMasternode::CalculateScore(int mod, int64_t nBlockHeight)
{
    ...
    if (!GetBlockHash(hash, nBlockHeight)) {
        printf("CalculateScore ERROR - nHeight %d - Returned 0\n", nBlockHeight);
        return 0;
    }
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        printf("mnb - Signature rejected, too far into the future %s\n", vin.ToString());
        nDos = 1;
        return false;
    }
    ...
    if (protocolVersion < masternodePayments.GetMinMasternodePaymentsProto()) {
        printf("mnb - ignoring outdated Masternode %s protocol version %d\n", vin.ToString(), protocolVersion);
        return false;
    }
    ...
    if (pubkeyScript.size() != 25) {
        printf("mnb - pubkey the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        printf("mnb - pubkey2 the wrong size\n");
        nDos = 100;
        return false;
    }

    if (!vin.scriptSig.empty()) {
        printf("mnb - Ignore Not Empty ScriptSig %s\n", vin.ToString());
        return false;
    }

    std::string errorMessage = "";
    if (!obfuScationSigner.VerifyMessage(pubKeyCollateralAddress, sig, strMessage, errorMessage)) {
        printf("mnb - Got bad Masternode address signature\n");
        nDos = 100;
        return false;
    }
    ...
    if (pmn == NULL)
        ...
        if (pmn->sigTime > sigTime) {
            printf("mnb - Bad sigTime %d for Masternode %20s %105s (existing broadcast is at %d)\n",
    ...
    if (pmn->pubKeyCollateralAddress == pubKeyCollateralAddress && !pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS)) {
        ...
        printf("mnb - Got updated entry for %s\n", addr.ToString());
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (GetInputAge(vin) < MASTERNODE_MIN_CONFIRMATIONS) {
        printf("mnb - Input must have at least %d confirmations\n", MASTERNODE_MIN_CONFIRMATIONS);
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        if (pConfIndex->GetBlockTime() > sigTime) {
            printf("mnb - Bad sigTime %d for Masternode %20s %105s (%i conf block is at %d)\n",
                sigTime, addr.ToString(), vin.ToString(), MASTERNODE_MIN_CONFIRMATIONS, pConfIndex->GetBlockTime());
            return false;
        }
    }

    printf("mnb - Got NEW Masternode entry - %s - %s - %s - %lli \n", GetHash().ToString(), addr.ToString(), vin.ToString(), sigTime);
    CMasternode mn(*this);
    mnodeman.Add(mn);
...
bool CMasternodeBroadcast::Sign(CKey& keyCollateralAddress)
{
    ...
    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, sig, keyCollateralAddress)) {
        printf("CMasternodeBroadcast::Sign() - Error: %s\n", errorMessage);
        return false;
    }
...
{
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        printf("CMasternodePing::CheckAndUpdate - Signature rejected, too far into the future %s\n", vin.ToString());
        nDos = 1;
        return false;
    }

    if (sigTime <= GetAdjustedTime() - 60 * 60) {
        printf("CMasternodePing::CheckAndUpdate - Signature rejected, too far into the past %s - %d %d \n", vin.ToString(), sigTime, GetAdjustedTime());
        nDos = 1;
        return false;
    }
    ...
    if (pmn != NULL && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
        ...
        if (!pmn->IsPingedWithin(MASTERNODE_MIN_MNP_SECONDS - 60, sigTime)) {
            ...
            if (!obfuScationSigner.VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, errorMessage)) {
                printf("CMasternodePing::CheckAndUpdate - Got bad Masternode address signature %s\n", vin.ToString());
                nDos = 33;
                return false;
            }
            ...
            if (mi != mapBlockIndex.end() && (*mi).second) {
                if ((*mi).second->nHeight < chainActive.Height() - 24) {
                    printf("CMasternodePing::CheckAndUpdate - Masternode %s block hash %s is too old\n", vin.ToString(), blockHash.ToString());
                ...
                if (fDebug) printf("CMasternodePing::CheckAndUpdate - Masternode %s block hash %s is unknown\n", vin.ToString(), blockHash.ToString());
